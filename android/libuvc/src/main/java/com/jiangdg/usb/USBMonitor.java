		...
		if (mPermissionIntent == null) {
			...
			if (context != null) {
				if (Build.VERSION.SDK_INT >= 31) {
					...
					// Fix for Android 14+: use explicit Intent (set package) to avoid
					// FLAG_MUTABLE + implicit Intent security exception
					int PENDING_FLAG_IMMUTABLE = 1<<25;
					Intent intent = new Intent(ACTION_USB_PERMISSION);
					intent.setPackage(context.getPackageName());
					mPermissionIntent = PendingIntent.getBroadcast(context, 0, intent, PENDING_FLAG_IMMUTABLE);
				} else {
